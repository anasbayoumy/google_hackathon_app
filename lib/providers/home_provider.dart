...
class HomeNotifier extends StateNotifier<HomeState> {
  ...
  Future<String> transcribeWavWithVosk(String wavPath) async {
    ...
    try {
      // Increase timeout to 30 seconds for Vosk processing
      final text = await platform.invokeMethod<String>(
        ...
        onTimeout: () {
          debugPrint('🎤 [VOSK] Transcription timed out after 30 seconds');
          return '';
        },
      );
      ...
      debugPrint('🎤 [VOSK] Text length: ${transcribedText.length} characters');
    ...
    try {
      ...
      if (state.audioPath != null && state.audioPath!.isNotEmpty) {
        ...
        if (recognizedText.isNotEmpty) {
          ...
          debugPrint(
              '✅ [ANALYZE] Using transcribed text as prompt: "$promptText"');
        } else {
          debugPrint('⚠️ [ANALYZE] Vosk transcription returned empty text');
        }
      } else {
        debugPrint('📝 [ANALYZE] No audio file, using text input only');
      }

      debugPrint('Calling AiService.runInference with text: \'' +
          promptText +
          '\', image: \'' +
          (state.imagePath ?? '') +
          '\', audio: \'' +
