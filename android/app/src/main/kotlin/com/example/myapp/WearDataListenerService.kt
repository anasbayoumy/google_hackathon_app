...
class WearDataListenerService : WearableListenerService() {
    ...
    private suspend fun sendErrorToWatch(error: String) {
        withContext(Dispatchers.IO) {
            try {
                Log.d(TAG, "📱 PHONE: Sending error to watch: $error")
                ...
                try {
                    ...
                    Log.d(TAG, "✅ PHONE: Error sent via DataClient")
                } catch (e: Exception) {
                    Log.w(TAG, "⚠️ PHONE: DataClient error send failed: ${e.message}")
                }

                // Also send via MessageClient
                try {
                    val connectedNodes = Wearable.getNodeClient(this@WearDataListenerService).connectedNodes.await()
                    for (node in connectedNodes) {
                        Wearable.getMessageClient(this@WearDataListenerService)
                            .sendMessage(node.id, "error", error.toByteArray()).await()
                    }
                    Log.d(TAG, "✅ PHONE: Error sent via MessageClient")
                } catch (e: Exception) {
                    Log.w(TAG, "⚠️ PHONE: MessageClient error send failed: ${e.message}")
                }

                Log.d(TAG, "✅ PHONE: Error sent to watch via both methods")
            } catch (e: Exception) {
                Log.e(TAG, "❌ PHONE: Failed to send error to watch", e)
            }
        }
    }
    ...
    private suspend fun sendSmsToWatch(nodeId: String, smsText: String) {
        withContext(Dispatchers.IO) {
            try {
                ...
                // Send via MessageClient (primary method)
                try {
                    Wearable.getMessageClient(this@WearDataListenerService)
                        .sendMessage(nodeId, SMS_RESPONSE_PATH, smsText.toByteArray()).await()
                    Log.d(TAG, "✅ PHONE STEP 17A: SMS sent via MessageClient")
                } catch (e: Exception) {
                    Log.w(TAG, "⚠️ PHONE STEP 17A: MessageClient failed: ${e.message}")
                }

                // Also send via DataClient (backup method)
                try {
                    val putDataRequest = PutDataRequest.create(SMS_RESPONSE_PATH).apply {
                        val dataMap = DataMap().apply {
                    ...
                    Wearable.getDataClient(this@WearDataListenerService)
                        .putDataItem(putDataRequest).await()
                    Log.d(TAG, "✅ PHONE STEP 17B: SMS sent via DataClient")
                } catch (e: Exception) {
                    Log.w(TAG, "⚠️ PHONE STEP 17B: DataClient failed: ${e.message}")
                }

                Log.d(TAG, "✅ PHONE STEP 17: SMS sent to watch via both methods")
            } catch (e: Exception) {
